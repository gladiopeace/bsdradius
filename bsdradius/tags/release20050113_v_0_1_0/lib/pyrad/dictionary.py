...
# HeadURL		$HeadURL: file:///Z:/backup/svn/bsdradius/tags/release20050113_v_0_1_0/lib/pyrad/dictionary.py $
# Author:		$Author: valts $
# File version:	$Revision: 92 $
# Last changes:	$Date: 2005-12-12 17:09:18 +0200 (Pr, 12 Dec 2005) $
...
class Dictionary:
	...
	# matches string which definately is not vendor name
	nonVendor = re.compile(r'^.*?(=|has_tag).*$')
	...
	def __ParseAttribute(self, state, tokens):
		...
		# get vendor name
		if len(tokens) >= 5 and not self.nonVendor.match(tokens[4]):
			vendor = tokens[4]
	...
	def ReadDictionary(self, file, parseValues = False, parseInclude = True):
		...
		debug ("Parsing dictionary file: ", file)
